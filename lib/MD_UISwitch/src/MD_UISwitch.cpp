...
*/
{
  ...
  const uint8_t TC_SHIFT = 3;   // bit shift for fraction
  const uint8_t TC = 31;         // RC time constant
  const uint8_t UTH = 238;      // Upper Threshold  for 'on'
  const uint8_t LTH = 12;        // Lower Threshold for 'off'
  // ---
